...
public class HBDpCmdLineBuilder extends DpCmdLineBuilder {
	@Override
	public String buildCmdLine(TaskLog runInfo) {
		...
		if (StringUtils.equals(runInfo.getProctype(),
				MsgType.taskTypeProc.name())||StringUtils.equals(runInfo.getProctype(),
						MsgType.SCOPE.name())) {
			cmdLine = new StringBuilder().append("sh ")
