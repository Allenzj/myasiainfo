...
public abstract class ScheduleExeInter {
	...
	public void runcmd(com.asiainfo.dacp.dp.message.DpMessage message,String cmdLine,DpAgentContext context){
		...
		String sourceQueue = msg.getSourceQueue();
...
//		command = finalCmdLine.split(" ");
		...
		// 执行命令
		String finalCmdLine = StringUtils.trim(args.toString());
		finalCmdLine = "java -jar  agentExetue/dacp-task-agentExectue-1.0.0.jar "+msgId+" "+finalCmdLine+" "+ context.getAgentCode()+" "+context.getLogPath() +"";
		command=finalCmdLine.split(" ");
		logger.info(finalCmdLine);
		try {
			execText += String
					.format("[%s,%s]正在执行\r\n", msgId, cmdLine);
			...
			context.getDpSender().sendMessage(sourceQueue, returnMsg);
			CmdExecutor executor = new CmdExecutor();
			logger.info("task[{}]执行命令[{}]",msgId,finalCmdLine);
		    int exitV = executor.execCmdNoLog(finalCmdLine);
		    returnMsg.getFirstMap().put(MapKeys.PROC_RETURN_CODE, String.valueOf(exitV));
		    if(exitV==0){
		    	returnMsg.getFirstMap().put(MapKeys.PROC_STATUS,
		    ...
		    returnMsg.getFirstMap().put(MapKeys.PROC_LOG, execText+executor.getLines()+"\r\n"+DpAgentUtils.getLog(logPath));
